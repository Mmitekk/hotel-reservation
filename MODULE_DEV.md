...
**Успешный ответ (200):** `{"success": true, "message": "…", "redirect": "/hotel-reservation/my-bookings"}` — пароль сохранён, гость автоматически залогинен, редирект в личный кабинет. Токен одноразовый. Для привязанных (не новых) аккаунтов токен не выдаётся — форма показывает подсказку со ссылкой на страницу входа (`account_linked: true` в ответе submit).
```
